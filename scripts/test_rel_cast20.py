...
def print_trec_res(run_file, qrel_file, rel_threshold, input_query_file):
    ...
    #mrr_one_nums = 0
    #mrr_zero_nums = 0
    res_dict = improve_judge(input_query_file, ndcg_3_list)
    #for key, value in res_dict.items():
        #if (len(value) > 0 and 1 in value[1:]) or len(value) == 1:
    #    if (len(value) > 0 and 1 in value):
    #        mrr_one_nums += 1
    #    else:
    ...
    with open(oj("output/cast20/dense_rel", "dev_rel_label_ndcg_rawq.json"), "w") as f:
        for key, value in res_dict.items():
            id_list = key.split('-')
            conv_id = id_list[0]
            turn_id = id_list[1]
...
def improve_judge(input_query_file, score_list):
    ...
    #embed()
    #input()
    assert len(data) == len(score_list)
    rel_label = {}
    rel_list = []
    base_score = 0
...
# python test_rel_cast20.py --config=Config/test_rel_cast20.toml