...
from data_structure import ConvDataset, ConvDataset_topiocqa
...
def train(args, log_writer):
    ...
    if args.continue_train_query_encoder:
        ...
        #query_tokenizer, query_encoder = load_new_model(args.model_type + "_Query", args.pretrained_query_encoder)
        query_encoder = query_encoder.to(args.device)
    ...
    # data prepare
    train_dataset = ConvDataset_topiocqa(args, query_tokenizer, passage_tokenizer, args.train_file_path)
    ...
    for epoch in epoch_iterator:
        ...
        for batch in tqdm(train_loader,  desc="Step", disable=args.disable_tqdm):
            ...
            if args.mode == "context":
                conv_query_embs = query_encoder(bt_raw_query, bt_raw_query_mask, bt_history_context, bt_history_context_mask, bt_conv_query, bt_conv_query_mask)
            elif args.mode == "context_fuse":
                conv_query_embs = query_encoder(bt_raw_query, bt_raw_query_mask, bt_history_context, bt_history_context_mask)
            else:
...
#  python train_topiocqa.py --config Config/train_topiocqa.toml