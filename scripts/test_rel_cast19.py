...
from data_structure import ConvDataset, ConvDataset_cast, ConvDataset_cast_rel
import os
from os import path
...
def get_test_query_embedding(args):
    ...
    test_dataset = ConvDataset_cast_rel(args, query_tokenizer, passage_tokenizer, args.test_file_path, add_doc_info=False)
...
def print_trec_res(run_file, qrel_file, rel_threshold, input_query_file):
    ...
    #print("ndcg_list")
    #print(ndcg_3_list)
    res_mrr_dict = improve_judge(input_query_file, ndcg_3_list)
    ...
    with open(oj("output/cast19/dense_rel", "dev_rel_label_ndcg_rawq.json"), "w") as f:
        for key, value in res_mrr_dict.items():
            id_list = key.split('-')
            conv_id = id_list[0]
            turn_id = id_list[1]
...
def improve_judge(input_query_file, score_list):
    ...
    for i, line in enumerate(data):
        ...
        #    rel_list = []
        if type_id == 0 and turn_id > 1: # 2
            base_score = score_list[i]
        elif type_id > 0 and turn_id > 1: # 2
...
if __name__ == '__main__':
    main()
    #print_trec_res("output/cast19/dense_rel/dev_dense_rel_res.trec", "datasets/cast19/qrels.tsv", 1, "datasets/cast19/dev_rel.json")

# python test_rel_cast19.py --config=Config/test_rel_cast19.toml