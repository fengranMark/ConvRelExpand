...
def main():
    ...
    merged_data = []
    with open(oj(args.output_dir_path, "train_rel_res.trec"), "w") as f:
    ...
    res = print_res(oj(args.output_dir_path, "train_rel_res.trec"), args.gold_qrel_file_path, args.rel_threshold, args.input_query_path)
    return res  


def print_res(run_file, qrel_file, rel_threshold, input_query_file):
    ...
    with open(oj("output/topiocqa/bm25", "train_rel_label_rawq_1.json"), "w") as f:
        for key, value in res_mrr_dict.items():
            id_list = key.split('-')
            conv_id = id_list[0]
            turn_id = id_list[1]
...
def improve_judge(input_query_file, score_list):
    ...
    for i, line in enumerate(data):
        ...
        conv_id = int(id_list[0]) # total 205
...
# python bm25_rel_topiocqa.py --config Config/bm25_rel_topiocqa.toml